...
def simplify_external(ast, name):
    def create_fn(vtype, i):
        if vtype[2][0] == ['->']:
            body = create_fn(vtype[2][2], i + 1)
        else:
            body = {
                'type': 'external',
                'name': ast['name'],
        ...
        return {
            'type': 'fn',
            'name': ast['name'] + ('' if i == 0 else '.' + str(i)),
            'arg': 'arg.' + str(i),
            'body': body,
        }

    return create_fn(ast['vtype'], 0)
...
def simplify_expression(ast, name):
    if ast['type'] == 'def':
        ...
        return simplify_external(ast, name)
    else:
        raise ValueError(f"Wrong ast type: {repr(ast['type'])}")
...
if __name__ == '__main__':
    ...
    simplified_ast = simplifier(ast)
    json.dump(simplified_ast, sys.stdout, indent=2)
    sys.stdout.write('\n')
